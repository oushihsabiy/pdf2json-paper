...
"""Convert book-style LaTeX (exercise/solution manuals) into example.json schema."""
...
def parse_section_like_title(title: str) -> Tuple[List[int], str]:
    """
    Parse section/exercise-like heading titles for ConvexOptimizationSolutionsManual style.
...
LATEX_TO_JSON_PROMPT_TEMPLATE = (
    "Convert ONE LaTeX snippet from a math EXERCISE/SOLUTION manual into structured JSON items.\n"
    ...
    "  - content: the exercise/problem statement in LaTeX (plain text or wrapped env is both acceptable)\n"
    "  - proof: either \"\" or COMPLETE \\begin{proof}...\\end{proof}\n\n"
    "Rules (strict):\n"
    "1) Never output env outside the allowed list. Never output env='text'.\n"
    "2) Prefer extracting exercise-like content (problem statement, subparts, and corresponding solution/proof).\n"
    "3) Preserve source fidelity: keep wording, formulas, numbering, qualifiers, and references.\n"
    "4) Do NOT hallucinate or complete missing math content.\n"
    "5) For solution-manual text:\n"
    "   - Put problem text into content.\n"
    "   - Put source solution/proof text into proof when explicitly present.\n"
    "   - If no explicit solution/proof is present, set proof to \"\".\n"
    "6) Drop non-problem noise (ToC, page headers/footers, figure/table remnants, prompt leakage).\n"
    "7) If nothing usable as an exercise item exists, output [].\n\n"
...
def statement_needs_refine(content: str, env_code: str) -> bool:
    ...
    if _NARRATIVE_CUES_RE.search(s):
        return True
    # Definitions/algorithms are more often narrative in textbooks.
...
REFINE_ITEM_PROMPT_TEMPLATE = (
    "You refine ONE extracted item from a LaTeX exercise/solution manual.\n"
    ...
    "- For solution-manual extraction, prefer keeping source statement and source proof unchanged.\n\n"
    "Proof requirements:\n"
...
def convert_tex_to_items(
    ...
    *,
    ...
    for u in tqdm(units, desc="LLM units", unit="unit"):
        ...
        # - for prose units, skip (non-formalizable)
        if not raw_items:
        ...
        for rit in raw_items:
            ...
            # For solution-manual style extraction:
            # proof ONLY comes from source tex (solution/proof). If source has none, keep proof empty.
            ...
            # If proof is missing but the env normally requires one, ask LLM to generate it.
            if (not lock_source_proof) and gen_missing_proofs and (not proof.strip()) and env0 in PROOF_REQUIRED_ENVS:
                proof = llm_generate_proof_for_statement(
                    client=client,
                    model=proof_model,
                    statement_env_text=content,
...
def to_example_output_schema(
    ...
    *,
    ...
    Tailored for ConvexOptimizationSolutionsManual-like exercise/solution PDFs.
    """
    ...
    def classify_problem_type(problem: str, proof: str) -> str:
        ...
        # Cues observed in ConvexOptimizationSolutionsManual.pdf:
