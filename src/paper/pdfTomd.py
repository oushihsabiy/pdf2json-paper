...
OCR_MD_PROMPT = """
You are doing OCR for an optimization research paper page (journal/conference).
...
2) Preserve title, authors, affiliations, abstract, keywords, and section/subsection headings.
3) Preserve theorem/lemma/definition/proposition/corollary/remark names and numbering.
4) Preserve algorithm/pseudocode blocks and figure/table captions when visible.
5) Preserve equation numbers, citation markers (e.g., [12], (Smith, 2020)), and footnotes.
6) If the page is two-column, read left column top-to-bottom, then right column.
7) Preserve math faithfully using standard Markdown math:
   - inline: $...$
   - display: $$...$$
8) Do NOT hallucinate or expand repeated content. If unreadable, keep minimal placeholder [illegible].
9) Output only page content, no markdown code fences.
""".strip()
...
Keep section headings, theorem/lemma numbering, and citations when visible.
Output only content.
""".strip()
